...
&#x200B;<!-- Build & Release --> [![CI](https://img.shields.io/github/actions/workflow/status/alimtvnetwork/macro-ahk-v26/ci.yml?branch=main&label=CI&logo=github&style=flat-square)](https://github.com/alimtvnetwork/macro-ahk-v26/actions/workflows/ci.yml) <!-- Repo activity --> [![Issues](https://img.shields.io/github/issues/alimtvnetwork/macro-ahk-v26?style=flat-square&logo=github)](https://github.com/alimtvnetwork/macro-ahk-v26/issues) [![Pull Requests](https://img.shields.io/github/issues-pr/alimtvnetwork/macro-ahk-v26?style=flat-square&logo=github)](https://github.com/alimtvnetwork/macro-ahk-v26/pulls) [![Repo Size](https://img.shields.io/github/repo-size/alimtvnetwork/macro-ahk-v26?style=flat-square)](https://github.com/alimtvnetwork/macro-ahk-v26)
&#x200B;<!-- Community --> <!-- (intentionally empty — see mem://constraints/no-static-mockup-badges) --> <!-- Code-quality --> [![Security Issues](https://img.shields.io/github/issues-search/alimtvnetwork/macro-ahk-v26?query=is%3Aissue%20label%3Asecurity&label=security&style=flat-square&logo=github)](https://github.com/alimtvnetwork/macro-ahk-v26/issues?q=is%3Aissue+label%3Asecurity) [![Dependency PRs](https://img.shields.io/github/issues-pr-raw/alimtvnetwork/macro-ahk-v26?label=dependency%20PRs&style=flat-square&logo=dependabot)](https://github.com/alimtvnetwork/macro-ahk-v26/pulls?q=is%3Apr+label%3Adependencies) <!-- Stack & standards --> [![License](https://img.shields.io/github/license/alimtvnetwork/macro-ahk-v26?style=flat-square)](#license)
...
```powershell
irm https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v26/main/scripts/install.ps1 | iex
```

### 🐧 macOS · Linux · Bash (latest)

```bash
curl -fsSL https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v26/main/scripts/install.sh | bash
```

### 📌 Pinned version (`v2.230.0`)

```powershell
# Windows · PowerShell
irm https://github.com/alimtvnetwork/macro-ahk-v26/releases/download/v2.230.0/install.ps1 | iex
```

```bash
# macOS · Linux · Bash
curl -fsSL https://github.com/alimtvnetwork/macro-ahk-v26/releases/download/v2.230.0/install.sh | bash
```

**Pinned version:** `v2.230.0` &nbsp;•&nbsp; **Macro Controller:** `v7.41`
...
- 🔒 Pinned to `v2.230.0` — see [Pinned to a specific release](#-pinned-to-a-specific-release) to change versions
...
- 🔒 Pinned to `v2.230.0` — see [Pinned to a specific release](#-pinned-to-a-specific-release) for other tags
...
```powershell
# PowerShell (Windows) — replace v2.230.0 with any released tag
irm https://github.com/alimtvnetwork/macro-ahk-v26/releases/download/v2.230.0/install.ps1 | iex
```
...
```powershell
& { $Version = "v2.220.0"; irm https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v26/main/scripts/install.ps1 | iex }
```

```bash
curl -fsSL https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v26/main/scripts/install.sh | bash -s -- --version v2.220.0
```
...
|------|-------------|---------|
| `-Version` | Pin a specific release (`vX.Y.Z[-pre]`) or `latest` | `-Version v2.230.0` |
| `-InstallDir` | Custom install directory | `-InstallDir D:\marco-extension\v2.230.0` |
| `-Repo` | Override GitHub `owner/repo` | `-Repo alimtvnetwork/macro-ahk-v26` |
...
|------|-------|-------------|---------|
| `--version <ver>` | `-v` | Force a specific version (`vX.Y.Z[-pre]`) or `latest`. Anything else exits 3. | `--version v2.230.0` |
| `--dir <path>` | `-d` | Target directory (default: `~/marco-extension`) | `--dir ~/marco-extension/v2.230.0` |
| `--repo <o/r>` | `-r` | Override GitHub `owner/repo` | `--repo alimtvnetwork/macro-ahk-v26` |
| `--dry-run` | — | Resolve the install plan, print it, exit 0 — **nothing is downloaded or extracted**. Useful for CI smoke tests and debugging. | `--dry-run --version v2.230.0` |
...
When the project ships successive major rewrites in sibling repos (e.g. `macro-ahk-v26` → `macro-ahk-v26` → `macro-ahk-v26`), discovery mode can optionally probe ahead and pick the newest. Defaults live in [`scripts/install.config.sh`](scripts/install.config.sh) and are **off by default**:
...
curl -fsSL https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v26/main/scripts/install.sh \
  | bash -s -- --dry-run

# Pin a specific version into a custom directory:
./install.sh --version v2.230.0 --dir ~/marco-extension/v2.230.0
...
1. Download `marco-extension-v{VERSION}.zip` from [Releases](https://github.com/alimtvnetwork/macro-ahk-v26/releases)
2. Extract to a folder (e.g., `D:\marco-extension\v2.230.0`)
...
```bash
git clone https://github.com/alimtvnetwork/macro-ahk-v26 "macro-ahk"
```
...
**No email or notification is sent** — check the [Releases page](https://github.com/alimtvnetwork/macro-ahk-v26/releases) for status.
...
```powershell
irm https://github.com/alimtvnetwork/macro-ahk-v26/releases/download/v{VER}/install.ps1 | iex
```

**Linux / macOS**

```bash
curl -fsSL https://github.com/alimtvnetwork/macro-ahk-v26/releases/download/v{VER}/install.sh | bash
```
